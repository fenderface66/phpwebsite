...
function add_case() { 
	...
	if (isset($_POST['create_case'])) {
		...
		$case_description = mysqli_real_escape_string($connection, $case_blurb);
...
function add_user() {
	...
	if (isset($_POST['create_user'])) {
		...
		if ($user_password == $confirm_password) {
			...
			$query = 
				"INSERT INTO users(username, user_password, user_firstname, user_lastname, role) ";
			$query .= 
				"VALUES('{$username}','{$hashed_password}','{$user_firstname}','{$user_lastname}','{$role}' ) ";
			...
			echo "<script>alert('Confirmed password does not match password, please try again')</script>";
		}
...
function find_cases() {
	...
	while($row = mysqli_fetch_assoc($select_cases)) {
		...
		echo "<td><a href='../casestudy.php?c_id={$case_id}'>View</a></td>";
		echo "<td><a onClick=\"javascript:confirm('Are you sure you want to delete this case?')\" href='case_studies.php?delete={$case_id}'>Delete</a></td>";
